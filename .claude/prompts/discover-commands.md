...
- **build**: The build command (e.g., `npm run build`, `cargo build`)
- **typecheck**: The type check command (e.g., `npx tsc --noEmit`, `mypy`)

Skip any command that shows `<placeholder>`, is commented out, or is empty.

Report which commands are available and which are not configured.
