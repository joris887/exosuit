...
- Profile gating via `lib/hook-guard.sh` and `EXOSUIT_HOOK_PROFILE` env var
...
- `EXOSUIT_PROJECT_PROFILE` (lean/standard/strict) — project complexity profile; controls skill behavior and derives default hook profile. Read from env var, then CLAUDE.md `**Profile:**` line, then defaults to standard.
- `EXOSUIT_HOOK_PROFILE` (minimal/standard/strict) — explicit hook profile override; if not set, derived from project profile (lean→minimal, standard→standard, strict→strict)
- `EXOSUIT_DISABLED_HOOKS` (comma-separated IDs) — skips individually disabled hooks
- `EXOSUIT_EXPLAIN_MODE` (off/brief/verbose) — controls message verbosity (brief = default)
- `EXOSUIT_STOP_MAX_ITERATIONS` — overrides stop hook safety valve (default 5, strict profile defaults to 10, ≤0 = no limit)
