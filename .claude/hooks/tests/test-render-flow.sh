...
check "gate rendered as hexagon" "1" "$(grep -c 'the_gate{{' r1.out || true)"
check "terminal rendered as circle" "1" "$(grep -c 'done((' r1.out || true)"
check "fanout renders both branches" "2" "$(grep -c '^  fan --> ' r1.out || true)"
check "STOP edge rendered" "1" "$(grep -c 'the_gate -->|fail| stop1' r1.out || true)"
...
bash "$RENDER" --write >/dev/null 2>&1
sed -i 's/next: the-gate/next: done/' .claude/skills/alpha/flow.yaml
RC=0; bash "$RENDER" --check >/dev/null 2>&1 || RC=$?
