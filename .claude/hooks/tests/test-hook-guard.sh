...
SAVED_PROJECT_PROFILE="${EXOSUIT_PROJECT_PROFILE:-}"
SAVED_HOOK_PROFILE="${EXOSUIT_HOOK_PROFILE:-}"
SAVED_DISABLED="${EXOSUIT_DISABLED_HOOKS:-}"
SAVED_MAX_ITER="${EXOSUIT_STOP_MAX_ITERATIONS:-}"

cleanup_env() {
    export EXOSUIT_PROJECT_PROFILE="$SAVED_PROJECT_PROFILE"
    export EXOSUIT_HOOK_PROFILE="$SAVED_HOOK_PROFILE"
    export EXOSUIT_DISABLED_HOOKS="$SAVED_DISABLED"
    export EXOSUIT_STOP_MAX_ITERATIONS="$SAVED_MAX_ITER"
}
...
export EXOSUIT_HOOK_PROFILE="standard" EXOSUIT_DISABLED_HOOKS="" EXOSUIT_PROJECT_PROFILE=""
...
unset EXOSUIT_HOOK_PROFILE
export EXOSUIT_PROJECT_PROFILE="lean" EXOSUIT_DISABLED_HOOKS=""
actual=0; sh "$GUARD" "test-hook" "standard" || actual=$?
...
export EXOSUIT_PROJECT_PROFILE="strict"
actual=0; sh "$GUARD" "test-hook" "strict" || actual=$?
test_case "Strict project → strict hooks: run strict" 0 "$actual"

# --- Hook disabling ---
echo ""
echo "  -- Hook disabling via EXOSUIT_DISABLED_HOOKS --"

export EXOSUIT_HOOK_PROFILE="standard" EXOSUIT_PROJECT_PROFILE=""

# Disable specific hook
export EXOSUIT_DISABLED_HOOKS="my-hook"
...
export EXOSUIT_DISABLED_HOOKS="hook-a,hook-b,hook-c"
...
echo ""
echo "  -- EXOSUIT_HOOK_PROFILE overrides project profile --"

export EXOSUIT_PROJECT_PROFILE="lean" EXOSUIT_HOOK_PROFILE="strict" EXOSUIT_DISABLED_HOOKS=""
actual=0; sh "$GUARD" "test-hook" "strict" || actual=$?
test_case "Lean project + strict hook override → run strict" 0 "$actual"

export EXOSUIT_PROJECT_PROFILE="strict" EXOSUIT_HOOK_PROFILE="minimal"
actual=0; sh "$GUARD" "test-hook" "standard" || actual=$?
