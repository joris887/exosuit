...
if [ "$FLOW_MODE" = "block" ]; then
    ...
    if [ -n "$FLOW_CURSOR" ]; then
        ...
        FC_STATUS=$(grep '^status:' "$SESSIONS_DIR/.failure-state.md" 2>/dev/null | head -1 | sed 's/^status:[[:space:]]*//')
        ...
        if [ "$FC_BRANCH" = "$FC_GIT_BRANCH" ] && [ "$FC_STATUS" = "active" ] && [ -f "$FC_FLOW_FILE" ]; then
            ...
            if [ -n "$FC_NODE_LINE" ] && ! printf '%s' "$FC_NODE_LINE" | grep -q 'type: terminal'; then
                NEW_ITER=$((ITERATION + 1))
                echo "$NEW_ITER" > "$ITER_FILE" 2>/dev/null
                printf 'Flow check before stopping:\n  - /%s is mid-flow at node '\''%s'\'' (EXOSUIT_FLOW_MODE=block). Finish the flow, run: sh .claude/hooks/lib/graph-state.sh clear %s, or resume later with /continue.\n' "$FC_FLOW" "$FC_NODE" "$FC_FLOW" >&2
                exit 2
            fi
        fi
    fi
