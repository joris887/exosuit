#!/usr/bin/env bash
# Compare test counts between current branch and main.
# Usage: bash scripts/test-count-delta.sh [test-command]
#
...
Examples:
  ...
  bash scripts/test-count-delta.sh -- npx jest --listTests
HELP
  exit 0
}
...
# Count tests on main (stash any changes)
STASH_NEEDED=false
...
MAIN_COUNT=$(eval "$TEST_CMD" 2>/dev/null || echo "0")
